# -*- coding: utf-8 -*-
"""
Created on Wed Feb 22 11:30:49 2023
...
# %% Paths to data dir.
dataDir = 'G:\Shared drives\HPL_Drive\ACL OpenCap Study\opencap-processing-main'

# G:\Shared drives\HPL_Drive\ACL OpenCap Study\opencap-processing-main
# 'C:\\Users\\danie\\anaconda3\\envs\\opencap-core'
...
videoParameters['0001']['originName'] = 'origin1.trc'
...
def computeMarkerDifferences(trialName, mocapDir, videoTrcDir, markersMPJE):
    print(trialName)
...
def syncVerticalVelocity(mocapData, videoData, mocapTRC, videoTRC):
    ...
    corVal, lag = utilsChecker.cross_corr(d_mocapY_sum, d_videoY_sum,
...
for subjectName in sessionDetails:
    ...
    for sessionName in sessionDetails[subjectName]:
        ...
        # markerDataFolders = glob.glob(os.path.join(subjectVideoDir,markerdataFolder + '/*/'))
        ...
        for poseDetector in poseDetectors:
            markerDataFolders.append(os.path.join(subjectVideoDir, markerdataFolder, poseDetector))
        ...
        for markerDataFolder in markerDataFolders:
            ...
            for camComboFolder in camComboFolders:
                ...
                for postDir in os.listdir(camComboFolder):
                    print('in for loop')
                    
                    #if '.ini' in postDir:
                        #continue
                    
                    if not 'PostAugmentation' in postDir:
                        continue

                    if postDir == 'PostAugmentation':
                        postAugmentationType = 'old'
                    else:
                        postAugmentationType = postDir[17:]
                    ...
                    for idxTrial, trialName in enumerate(trialNames):
                        ...
                        try:
                            ...
                            MPJE_offsetRemoved_std[idxTrial], MPJE_markers[idxTrial, :], MPJE_offsetRemoved_markers[
                                ...
                                trialName, mocapDir, videoTrcDir, markersMPJE)
